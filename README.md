...
This plugin enables you to use a new syntax for Knockout’s built-in bindings `attr`, `css`, `sytle`, and `event`. This new syntax combines the names of the binding and the item you want to bind without having to use a sub-object.
...
When combined with [Knockout-Freedom](https://github.com/mbest/knockout-freedom), this syntax ensures that each of multiple attributes (etc.) is only updated when its own dependencies change. This is especially important when updating an attribute has side effects or involves some significant amount of work. For example, in the first case, an update to `myId` will update both bindings and cause the `iframe` to reload; but in the second case, only an update to `baseUrl` will cause it to reload:
...
This plugin works by dynamically creating a binding handler each time you use a new "key.subkey" binding. It does this by wrapping two Knockout functions, `ko.bindingProvider.instance.getBindings` (used for most bindings) and `ko.applyBindingsToNode` (used for bindings within string-based template such as jQuery-tmpl).

#### Using this syntax for custom bindings

If your binding follows the same pattern as the supported built-in bindings, it will just work automatically with the new syntax. Here’s an example binding:
...
This plugin exports `ko.keySubkeyBinding.makeBinding`, which can be used to manually generate a "key.subkey" binding handler. If both saves the handler in `ko.bindingHandlers` as "key.subkey" and returns the handler object. If the specified binding was already created before, `makeBinding` will create a new one, overwriting the old one. You can use this, for example, to create an alias for a binding:

    ko.bindingHandlers.valueOneWay = ko.keySubkeyBinding.makeBinding('attr.value');

This plugin also exports `ko.getBindingHandler` with similar functionality to `makeBinding`. But `getBindingHandler` will simply return, rather than re-create, an existing binding.

As described in the section about custom bindings, when trying to dynamically create an `x.y` (for example) binding handler, this plugin will call `ko.bindingHandlers.x.makeSubkeyHandler('x', 'y', 'x.y')` if the function exists, which should return a binding handler object (with `init` and/or `update` functions).
